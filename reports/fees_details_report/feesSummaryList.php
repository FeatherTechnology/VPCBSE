...
<table class="table table-bordered" id="show_student_fees_summary_list">
    <thead>
        <tr><th colspan='9'>Fees Summary Report From <?php echo $feesFromDate->format('d-m-Y'); ?>  To  <?php echo $feesToDate->format('d-m-Y'); ?> </th></tr>
        <tr>
            <th>S.No</th>
            <th>Date</th>
            <th>School Fee</th>
    ...
    $getCollectedFeesQry = $connect->query("SELECT COALESCE(SUM(afd.fee_received),0) AS collectedFees ,    SUM(
            ...
            WHERE af.receipt_date ='$from_date' AND afd.fees_table_name ='grptable' AND af.school_id = '$school_id' ");
    ...
    $getBookFeesQry = $connect->query("SELECT COALESCE(SUM(afd.fee_received),0) AS bookFees ,    SUM(
            ...
            SUM(
             ...
             LEFT JOIN 
                admission_fees_denomination afd_deno ON af.id = afd_deno.admission_fees_ref_id
            WHERE af.receipt_date ='$from_date' AND afd.fees_table_name ='amenitytable' AND af.school_id = '$school_id' ");
    ...
    $getTransportFeesQry = $connect->query("SELECT COALESCE(SUM(tafd.fee_received),0) AS transportFees ,    SUM(
            ...
            SUM(
                CASE 
                    ...
                    ELSE 0 
                END
            ) AS bank_balance FROM `transport_admission_fees` taf JOIN transport_admission_fees_details tafd ON taf.id = tafd.admission_fees_ref_id 
             LEFT JOIN 
                transport_admission_fees_denomination tafd_deno ON taf.id = tafd_deno.admission_fees_ref_id
            WHERE taf.receipt_date ='$from_date' AND taf.school_id = '$school_id' ");
    ...
    $getLastyearFeesQry = $connect->query("SELECT COALESCE(SUM(lyfd.fee_received),0) AS lastyearFees ,    SUM(
            ...
            SUM(
                CASE 
                    ...
                    ELSE 0 
                END
            ) AS bank_balance FROM `last_year_fees` lyf JOIN last_year_fees_details lyfd ON lyf.id = lyfd.admission_fees_ref_id 
     LEFT JOIN last_year_fees_denomination lyfd_deno ON
            lyf.id = lyfd_deno.admission_fees_ref_id WHERE lyf.receipt_date ='$from_date' AND lyf.school_id = '$school_id' ");
